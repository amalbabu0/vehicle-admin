...
export function ListerSettingsForm({ name, phone, email }: { name: string | null; phone: string | null; email: string }) {
  ...
  return <div className="max-w-2xl space-y-5">
    ...
    <section className="space-y-3"><div><h2 className="text-base font-semibold">Security</h2><p className="text-sm text-muted-foreground">Changes require your current password.</p></div>
      <Card><CardHeader><CardTitle>Change Password</CardTitle><CardDescription>Use at least 8 characters with a letter, number, and special character.</CardDescription></CardHeader><CardContent><form action={passwordAction} className="space-y-4"><PasswordInput id="currentPassword" name="currentPassword" label="Current Password" autoComplete="current-password" /><PasswordInput id="newPassword" name="newPassword" label="New Password" autoComplete="new-password" value={newPassword} onChange={setNewPassword} /><PasswordStrength password={newPassword} /><PasswordInput id="confirmPassword" name="confirmPassword" label="Confirm New Password" autoComplete="new-password" /><Feedback {...passwordState} /><SaveButton pending={passwordPending}>Update Password</SaveButton></form></CardContent></Card>
      <Card><CardHeader><CardTitle>Change Email</CardTitle><CardDescription>Your current email is {email}. We will send confirmation instructions to complete the change.</CardDescription></CardHeader><CardContent><form action={emailAction} className="space-y-4"><div className="space-y-2"><Label htmlFor="email">New Email</Label><Input id="email" name="email" type="email" autoComplete="email" required aria-invalid={emailState?.field === "email"} /></div><PasswordInput id="emailCurrentPassword" name="currentPassword" label="Current Password" autoComplete="current-password" /><Feedback {...emailState} /><SaveButton pending={emailPending}>Change Email</SaveButton></form></CardContent></Card>
    </section>

    <section className="space-y-3"><div><h2 className="text-base font-semibold">Account</h2><p className="text-sm text-muted-foreground">This action is permanent.</p></div>
      <Card className="border-destructive/30"><CardHeader><CardTitle>Delete Account</CardTitle><CardDescription>Deleting your account permanently removes your lister profile and listings. Type DELETE and enter your password to continue.</CardDescription></CardHeader><CardContent><form action={deleteAction} className="space-y-4"><div className="space-y-2"><Label htmlFor="confirmation">Confirmation</Label><Input id="confirmation" name="confirmation" placeholder="DELETE" required aria-invalid={deleteState?.field === "confirmation"} /></div><PasswordInput id="deleteCurrentPassword" name="currentPassword" label="Current Password" autoComplete="current-password" /><Feedback {...deleteState} /><Button type="submit" variant="destructive" disabled={deletePending}>{deletePending && <LoaderCircle className="size-4 animate-spin" />}{deletePending ? "Deleting…" : "Delete Account"}</Button></form></CardContent></Card>
    </section>
  </div>;
}
